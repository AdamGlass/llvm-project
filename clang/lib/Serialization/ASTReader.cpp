...
void ASTReader::InitializeContext() {
  ...
  for (const auto &Entry : llvm::ArrayRef(SectionInfos)) {
    Context.SectionInfos[Entry.SectionName] = ASTContext::SectionInfo(
        nullptr, Entry.PragmaSectionLocation, Entry.SectionFlags);
  }
...
void ASTReader::UpdateSemaPragmaSegment(
    PragmaSegmentStackState &StackState,
    Sema::PragmaStack<std::string> &PragmaStack) {
  if (StackState.PragmaCurrentValue) {
    ...
    if (!StackState.PragmaStack.empty() &&
      ...
      assert(StackState.PragmaStack.front().Value == PragmaStack.DefaultValue &&
             "Expected a default section value");
      PragmaStack.Stack.emplace_back(
          StackState.PragmaStack.front().SlotLabel, PragmaStack.CurrentValue,
    ...
    for (const auto &Entry :
      ...
      PragmaStack.Stack.emplace_back(Entry.SlotLabel, Entry.Value,
                                     Entry.Location, Entry.PushLocation);
    }
    if (StackState.PragmaCurrentLocation.isInvalid()) {
      assert(StackState.PragmaCurrentValue == PragmaStack.DefaultValue &&
             "Expected a default section value");
      // Keep the current values.
    } else {
      PragmaStack.CurrentValue = *StackState.PragmaCurrentValue;
      PragmaStack.CurrentPragmaLocation = StackState.PragmaCurrentLocation;
    }
  }
}
