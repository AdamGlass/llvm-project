...
void Sema::AttachSectionFlags(VarDecl *VD, StringRef SectionName) {
  ...
  if (SectionIt != Context.SectionInfos.end()) {
    ...
    VD->addAttr(
        SectionFlagsAttr::CreateImplicit(Context, Section.SectionFlags));
  } else {
    Diag(VD->getLocation(), diag::err_section_unknown)
        << VD->getName() << SectionName;
  }
}
...
void Sema::ActOnPragmaMSSeg(SourceLocation PragmaLocation,
  ...
  if (SegmentName) {
    Stack->Act(PragmaLocation, Action, StackSlotLabel,
               std::string(SegmentName->getString()));
  } else {
...
void Sema::ActOnPragmaMSInitSeg(SourceLocation PragmaLocation,
  ...
  CurInitSeg =
      SegmentName->getString() == ".CRT$XCU" ? "" : SegmentName->getString();
...
void Sema::ActOnPragmaMSAllocText(
  ...
  UnifySection(Section, ASTContext::PSF_Read | ASTContext::PSF_Execute,
               PragmaLocation);
